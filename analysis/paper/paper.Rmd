...
# normalize <- function(x) (x-min(x))/(max(x)-min(x)) # between 0-1
normalize <- function(x) (1 + (x-min(x)))/(max(x)-min(x))
...
pc1_size_reg <- 
  ggscatter(jpgs_imported_coo_size_pcs_metric, 
    ...
    stat_cor(label.y = 2.25, label.sep = "\n") +
    #stat_regline_equation(label.y = 1) +
    labs(x = "", y = "PC1") +
    theme(axis.text = element_text(size = 8)) +
    #scale_x_continuous(breaks = c(75, 200)) +
    scale_y_continuous(limits = c(1, 2.5),
                       breaks = c(seq(1, 2.5, 0.5)),
                       labels = c(seq(1, 2.5, 0.5)))
