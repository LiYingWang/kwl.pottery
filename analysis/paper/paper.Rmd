...
Geometric morphometry methods (GMM) has been widely applied recently in archaeology for shape analysis, which explores morphological variability and similarity of archaeological materials to address the questions of anthropological interests. Different from linear measurement approach that captures shapes by measuring distances, angles or ratios of objects, geometric morphometrics methods use Cartesian coordinates of morphological structures to define the shapes [@Adams2004; @Lawing2010; @Slice2007]. Landmarks, curves or outlines of objects can be transformed into coordinates by their unique location and orientation with respect to numerical values of coordinate axes . According to different focuses and forms of original shape data, there are two common morphometric methods used to capture and analyze the shapes, landmark analysis and outline analysis [@Adams2004]. Landmark approaches assign a set of landmark points onto specimens that can be specified on a coordinate system. Each point has a coordinate represented by numerical values with respect to defined coordinate axes. Based on the distances and angles of interlandmarks, the objects are translated, rotated, and scaled into the same orientation and size using generalized Procrustes analysis (GPA). GPA method enables the comparison between shapes because it brings the landmark configurations of all objects into a common coordinate system, where differences are reflected based on landmark coordinate values [@Slice2007].

- related to the issue of specialization  
- CV is hard to test, we apply some statistics testing for it

# Hypotheses and expectations
The goal of my data collection for locally pottery is to explore whether pottery production could reflect the social change, from a more corporate mode to a more network mode after European contact. Craft specialization usually appears when the presence of elite control. In this case, if such feel of competitions for prestige and wonderful European resources among proud individuals gradually lead to change in social organization, then I expect the production of pottery will show more homogeneous features after European contact due to the craft specialization and the inspiration of the Europeans advisor. The standardization of metric attributes for pottery has being used to measure the craft specialization.
