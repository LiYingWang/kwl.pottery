...
Geometric morphometry methods (GMM) has been widely applied recently in archaeology for shape analysis, which explores morphological variability and similarity of archaeological materials to address the questions of anthropological interests. Different from linear measurement approaches that capture shapes by measuring distances, angles or ratios of objects, geometric morphometrics methods use Cartesian coordinates of morphological structures to define the shapes [@Adams2004; @Bookstein1997; @Lawing2010; @Slice2007]. Landmarks, curves or outlines of objects can be represented by coordinates with their unique location and orientation with respect to numerical values of coordinate axes. According to different focuses and forms of original shape data, there are two common morphometric methods used to capture and analyse the shapes, landmark approaches and outline approaches [@Adams2004]. Landmark approaches assign a set of landmarks onto specimens as reference points that can be specified on a coordinate system as x, y coordinates for two dimensions or x, y, z coordinates for three dimensions. Based on the distances and angles of landmark points, the objects are translated, rotated, and scaled into the same orientation and size using generalized Procrustes analysis (GPA) [@Bookstein1991]. GPA method extracts shape information and compares shapes by taking sets of landmark configurations into a common coordinate system, where coordinate values represent shape variables that can be further analysed by further multivariate statistical analyses [@Slice2007]. Landmark approach is a useful method for archaeological objects with extreme morphological signature or distinctive curvature that provide suitable locations for landmark placement, such as projectile point tips or biologically meaningful points indicating osteological differences [@Birch2019; @Buchanan2019; @Lycett2013; @Cardillo2010; @Haruda2019; @Meloro2015]. 

However, landmark approaches have a limitation that a set of landmarks may not be able to capture the shape differences of a morphological structure when the curve between landmarks conveys more important information than landmarks themselves. In addition, suitable locations for landmark placement may not be available for some complex or rounded shaped structures, such as pottery. 
