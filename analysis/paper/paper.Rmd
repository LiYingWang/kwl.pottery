...
jpgs_imported_coo_size <-
  jpgs_imported_coo %>% 
  mutate(size =  coo_centsize(jpgs_imported_coo)) 

# create pcs dataframe for joining
...
jpgs_imported_coo_size_df <-
  as.data.frame(jpgs_imported_coo_size$fac) %>% 
...
ggplot(jpgs_imported_coo_size_df,
       aes(size, fct_rev(phase))) +
  coord_flip() +
  geom_boxplot() +
  labs(x = "phase",
       y = "centroid size")
...
linearMod <- lm( size ~ pc1, data = jpgs_imported_coo_size_df)

ggplot(jpgs_imported_coo_size_df, 
       aes(size, pc1)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~ fct_rev(phase)) +
