...
Geometric morphometrics differs from traditional linear measurements through its use of Cartesian coordinates of morphological structures to define and analyze shape [@Adams2004; @Bookstein1997; @Lawing2010; @Slice2007]. Landmarks, curves or outlines of objects can be represented by coordinates in terms of their unique point locations with respect to numerical values on coordinate axes. There are two common morphometric methods: landmark and outline approaches [@Adams2004]. Landmark GM approaches assign a set of landmarks and/or semilandmarks onto objects as reference points that can be specified on a coordinate system. Landmark-based morphometrics are widely applied to archaeological objects with obvious morphological features that provide unambiguous reference points for landmark placement, such as tips and edges of stone tools or iron weapons[@Birch2019; @Buchanan2019; @Cardillo2010; @Lycett2013; @Selden2018], visually distinctive bone features [@Haruda2019; @Meloro2015], or ceramic assemblages with clear distinct components [@Selden2019; @Topi2017].

Geometric morphometrics methods can provide a useful framework for further statistical evaluation on shape variability of archaeological assemblages to explore how they change over time or across geographic areas to answer questions related to variation in morphology and standardization. A common procedure is using multivariate statistical methods, such as principal component analysis or canonical variate analysis, to capture the important variances of the overall variation in shape variables generated by GM and then visualize the variances to identify any clustered analytical groups that suggest less variability or high standardization. For example, the application of GM to stone tools coupled with statistical tests, such as coefficient of variation on principal component scores [@Archer2015], and on a series of metric variables including length, width, or thickness [@Buchanan2018; @Okumura2014; @Smith2016], or Kruskal-Wallis test on flake scar patterns [@Eren2015], which allows further discussion of causes of standardization and variability that may relate to degree of tool reduction, specialized function, or lack of innovation in a small group. In addition, cluster analysis such as k-mean aggregation for shape variables generated by GM is another way to discuss the variability among shape groups [@Doyon2019]. 

<!--discussion of standardization in material culture using GM approaches-->

However, a key limitation of the landmark approach is that landmarks may be difficult to reproducibly locate for structures that are mostly or entirely curves if not mathematically-defined. In those cases, Elliptic Fourier Analysis (EFA) is an excellent mean for assessing morphological variation in whole structure of two-dimensional closed shapes. EFA was first introduced to study shapes of artifacts in archaeology in 1980s [@Gero1984]. Most applications are associated with human remains and animal bones to study biological variation, and has been increasing for stone artifacts to explore stone tool topology and technology [@Fox2015; @Hoggard2019; @Iovictua2010]. Despite the EFA applications for ceramics is rare, this approach is promising for answering questions related to ceramic taxonomy [@Topi2017; @Wilczek2014]. For example, @Wilczek2014 evaluate the concordance between EFA and Discrete Cosine Transform (DCT), and a traditional typology by studying 154 complete ceramic vessels with varied shapes from the Bibracte oppidum in France. The results show the variation indicated by EFA and DCT matches the traditional ceramic typology, which supports that claim that outline-based approaches can be efficiently used for studying variations in ceramic shapes. Furthermore, @Wilczek2014's EFA results help us understand the level of production standardization over time across the region. 
