...
Geometric morphometry methods (GMM) has been widely applied recently in archaeology for shape analysis, which explores morphological variability and similarity of archaeological materials to address the questions of anthropological interests. Different from linear measurement approach that captures shapes by measuring distances, angles or ratios of objects, geometric morphometrics methods use Cartesian coordinates of morphological structures to define the shapes. Landmarks, curves or outlines of objects can be transformed into coordinates by their unique location and orientation with respect to numerical values of coordinate axes [@Adams2004; @Lawing2010; @Slice2007]. According to different focuses and forms of original shape data, there are two common morphometric methods used to capture and analyze the shapes, landmark analysis and outline analysis [@Adams2004]. Landmark approaches assign a set of landmark points to specimen that can be specified on a coordinate system. The landmark coordinates are represented by numerical values with respect to defined coordinate axes. Based on the specific position of each point, objects are translated, rotated, and scaled into the same orientation and size using Procrustes superimposition technique to find the best fit between configuration shapes [@Lawing2010]. Landmarks 

In archaeology, the landmarks are according to “type two” landmarks of Bookstein (1991), which define them as points located in the maximum of curvature or extreme points in morphology (Bookstein 1991; Zelditch et al. 2004).

were chosed  be applied to 3D objects not just limited to 2D that commonly applied to zooarchaeology and 




the analysis of shape coordinates generated by the least-squares superimposition of configurations of landmarks
