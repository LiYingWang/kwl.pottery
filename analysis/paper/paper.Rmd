...
Whether standardization suggests an emergence of specialized production depends on the number of producers, which distinguishes mechanical standardization from intentional standardization defined by stylish and functional attributes [@Costin2001; @Costin1995]. In other words, mechanical standardization is associated with potter's skills and experiences that increased morphological uniformity normally reflects fewer producers. Population could be an indicator to estimate the potential number of producers. According to the historical records [@Nakamura1938, pp. 12], the population in Kiwulan reached a higher number in the 17th century as the European presence and declined in the Chinese period due to movement of Indigenous people to the south [@Chen2007]. The changes in population size described in documents corresponds to the observation of ceramic abundance at Kiwulan. Thus, we model cv values as a function of the mass of ceramics using Poisson GLM with a link function. The model suggests that ceramic abundance strongly predicts the cv values (β = $`r beta_print`$, p = $`r pval_print`$). This indicates that a more standardized shape in the Chinese period may be influenced by a small population if ceramic abundance can reflect the population size. 
