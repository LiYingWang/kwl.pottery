...
```{r size-analysis-prep-2}
# combine with cv information 
size_pcs_cv_join <-
  jpgs_imported_coo_size_pcs_metric %>% 
  group_by(y) %>% 
  mutate(Diameter1_Body_ave = round(mean(Diameter1_Body), 2),
         cv_body_diameter = round(sd(Diameter1_Body)/mean(Diameter1_Body), 
                                  digits = 2)*100) %>%
  mutate(phase = as.factor(y)) %>% 
  left_join(cv_pcs_all) %>% 
  left_join(pc1_df_with_filename)
  #distinct(phase, cv_pc1, cv_pc2, cv_pc3, Diameter1_Body_ave, cv_body_diameter)
```

```{r boxplot-size-by-phase}
size_cv_df <-
  size_pcs_cv_join %>% 
  distinct(phase, Diameter1_Body_ave, cv_body_diameter, cv_pc1) %>% 
  mutate(cv_body_diameter_label = paste("CV=", cv_body_diameter, "%"),
         Diameter1_Body_ave_label = paste("mean=", Diameter1_Body_ave))
...
<<<<<<< HEAD
  annotate("text", x = c(126,122,145), y = c(2,1,3), 
           size = 3, label = cv_diameter_label) +
  theme_minimal()
```

```{r boxplot-pc1-ave-size}
boxplot_ave_size_cv_pc <-
  size_cv_df %>% 
  ggplot(aes(cv_body_diameter, cv_pc1)) +
  geom_point() +
  geom_text(label = size_cv_df$phase,
            size = 3.5,
...
=======
  theme_bw()
>>>>>>> ed52b8c815c2c9003bae727b45f8e1e52fb373d5
```


  scale_y_continuous(limits = c(10, 35),
                       breaks = c(seq(10, 35, 5)),
                       labels = c(seq(10, 35, 5))) +
...
```{r size-regression-plot-together}
# plot boxplot and regression together 
left_column <-
  plot_grid(boxplot_size, boxplot_ave_size_cv_pc, 
