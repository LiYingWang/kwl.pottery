 ...
 function changeZoom(val)
 {
  ...
  if(plotOptions.scale>8){plotOptions.scale=8;};
  if(plotOptions.scale==0){plotOptions.scale=1;};
  if(plotOptions.scale<(1/8)){plotOptions.scale=1/8;};
  readback();
 };
 ...
 function getMapParams()
 {
  ...
  sel=document.getElementById('mapCategory');
  plotOptions.category=sel.options[sel.selectedIndex].text;
  parent.findMapMinMax();
 ...
 function setMapParams()
 {
  ...
  sel=document.getElementById('mapCategory');
  sel.options.length=0;
  for(i=0;i<plotOptions.categories.length;i++)
  {
   sel.options[i]=new Option(plotOptions.categories[i],i);
   if(plotOptions.categories[i]==plotOptions.category){sel.selectedIndex=i;};
  };
 };
 ...
 function fileClick(i)
 {
  ...
  var newdir;
  var login="..\/login\/login.php";
  if(parent.location.pathname.indexOf('\/beta\/')!=-1){login="..\/"+login;};
  switch(i)
  {
  case 0:
   parent.newFile();
  ...
  case 8:
   if(parent.onAServer())
   {
    parent.location.replace(login+'?Action=Logout&Change=true&Location='
    	+encodeURIComponent(parent.location.pathname));
   };
   break;
  case 9:
   if(parent.onAServer())
   {
    parent.location.replace(login+'?Action=Logout&Location='
    	+encodeURIComponent(parent.location.pathname));
   };
   break;
  case 11:
