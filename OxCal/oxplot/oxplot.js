...
var colors=new Array("rgb(255,0,0)","rgb(0,255,0)","rgb(0,0,255)","rgb(204,204,0)", "rgb(204,0,204)","rgb(0,204,204)","rgb(127,127,127)");
   ...
   plotInfo.dx_calc="";
   plotInfo.dy_calc="";
   plotInfo.xlabel="Longitude";
   plotInfo.ylabel="Latitude";
   plotInfo.title="";
   plotInfo.category="";
   plotInfo.table="";
...
//    cont=cont.replace(/[^\u000A\u000D\u0020-\u007F]/g,"_");
 ...
 function keyHandler(e,t)
 {
  ...
  if(e.altKey){incr=1.002711275050202;};
  switch(t)
  {
  case "zoomMap":
   switch(e.keyCode)
  ...
  case "centreMap":
   ...
   {
   case 37: // left
    if(e.shiftKey){changeXRange(1/incr);}else{changeMinX(1-incr);};break;
   case 38: // up
    if(e.shiftKey){changeYRange(incr);}else{changeMinY(incr-1);};break;
   case 39: // right
    if(e.shiftKey){changeXRange(incr);}else{changeMinX(incr-1);};break;
   case 40: // down
    if(e.shiftKey){changeYRange(1/incr);}else{changeMinY(1-incr);};break;
   case 173:
   case 189: // -
    if(e.shiftKey){zoom(1/incr);}else{changeXRange(incr);if(!plotInfo.mapPlot){changeYRange(incr);};};break;
   case 61:
   case 187: // +
    if(e.shiftKey){zoom(incr);}else{changeXRange(1/incr);if(!plotInfo.mapPlot){changeYRange(1/incr);};};break;
   case 48: // 0
